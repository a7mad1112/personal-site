 ...
 *
 * Props:
 * - observeRef?: React.RefObject<HTMLElement>  // optional target to observe. If not provided, observes itself.
 * - className?: string                          // extra classes for the wrapper
 * - opacity?: number                            // wrapper opacity (0..1)
 * - duration?: number                           // animation duration in ms (default 1600)
 * - once?: boolean                              // animate once then keep drawn (default true)
 *
 * Notes:
 * - Uses unique IDs for gradients/masks to avoid collisions when multiple instances are on the page.
 * - Pointer-events are disabled so it never blocks clicks beneath.
 */
export default function AnimatedSvgLines({
  ...
  const uid = useId().replace(/:/g, ""); // stable unique suffix

  const maskId = `SvgMask-${uid}`;
  const gradOrangeId = `grad-orange-${uid}`;
  const gradReverseId = `grad-rev-${uid}`;

  useEffect(() => {
    const target = observeRef?.current ?? localRef.current;
    if (!target) return;
  ...
  return (
    <div
      ref={localRef}
      className={`absolute inset-0 z-0 pointer-events-none flex items-center justify-center w-full ${className}`}
      style={{ opacity }}
    >
      <svg
        ...
        // Pass duration via CSS var
        style={{ "--draw-duration": `${duration}ms` }}
      >
        <defs>
          <mask id={maskId}>
          ...
          <linearGradient id={gradOrangeId} x1="100%" y1="35%" x2="0%" y2="0%">
            <stop stopColor="#fb923c" offset="0%" />
            <stop stopColor="#f4280d" offset="100%" />
          </linearGradient>

          <linearGradient id={gradReverseId} x1="0%" y1="0%" x2="100%" y2="35%">
            <stop stopColor="#fb923c" offset="0%" />
            <stop stopColor="#f4280d" offset="100%" />
          </linearGradient>
      ...
      <style>{`
        .svg-draw path {
          /* pathLength="1" lets us use unit length 1 for clean control */
          stroke-dasharray: 1;
          stroke-dashoffset: 1;
          transition: stroke-dashoffset var(--draw-duration, 1600ms) ease-out;
        }
