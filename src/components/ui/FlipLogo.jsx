...
export default function FlipLogo({ src, title }) {
  return (
    <span className="block h-[var(--logoloop-logoHeight)] w-[var(--logoloop-logoHeight)] [perspective:800px]">
      {/* Rotating plane */}
      <span className="relative block h-full w-full transition-transform duration-700 [transform-style:preserve-3d] group-hover/item:[transform:rotateY(180deg)]">
        {/* Front */}
        <span
        ...
        {/* Back */}
        <span
          className="absolute inset-0 grid place-items-center rounded-2xl border border-[var(--accent)] bg-[var(--surface)]/80 text-[10px] font-medium text-white [transform:rotateY(180deg)]"
          style={{
            backfaceVisibility: "hidden",
            WebkitBackfaceVisibility: "hidden",
          }}
